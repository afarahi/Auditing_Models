...
def example_1():
    ...
    X, y = datasets.make_classification(n_samples=60000, n_features=20,
    ...
    for clf, name in [(lr, 'Logistic'),
        ...
        prob_cal = calibrate(y_cv, prob_cv, prob_test=prob_pos, method='isotonic')
        ...
        MLCE2 = MLCE2_test_estimator(X_test, y_test, prob_pos, prob_kernel_wdith=0.1, kernel_function='rbf', gamma=gamma)
        ax3.plot(MLCE2 * 100, 1, 'v', markersize=14, markeredgewidth=2)

        MLCE2 = MLCE2_test_estimator(X_test, y_test, prob_cal, prob_kernel_wdith=0.1, kernel_function='rbf', gamma=gamma)
    ...
    ax3.set_xlabel(r"MLCE$^2_{u}$", size=18)
    ax3.set_yticks([])
    # ax3.set_xlim([0.0, 50.])
    ...
    ax6.set_xlabel(r"MLCE$^2_{u}$", size=20)
    ax6.set_yticks([])
    # ax6.set_xlim([0.0, 50.])
...
def load_wdbc():
    ...
    "Decision Tree Construction Via Linear Programming." Proceedings of the 4th Midwest Artificial Intelligence
    and Cognitive Science Society, pp. 97-101, 1992], a classification method which uses linear programming to
    construct a decision tree. Relevant features were selected using an exhaustive search in the space of
...
def example_3_wf():
    ...
    def plot_ewf(k, prob_cv, prob_test, prob_test_grid, method, ax):
        ...
        bias1 = local_bias_estimator(X_test, y_test, prob, Xgrid, prob_grid, model='KRR', prob_kernel_wdith=0.1, kernel_function='rbf', alpha=0.1,  gamma=gamma/40.0)
        bias2 = local_bias_estimator(X_test, y_test, prob, Xgrid, prob_grid, model='KRR', prob_kernel_wdith=0.1, kernel_function='rbf', alpha=1.0, gamma=gamma/40.0)
        bias3 = local_bias_estimator(X_test, y_test, prob, Xgrid, prob_grid, model='KRR', prob_kernel_wdith=0.1, kernel_function='rbf', alpha=10.0, gamma=gamma/40.0)
...
def assess_calibration_model():
    ...
    plt.tight_layout()
    plt.savefig('./plots/calibration_model.png', bbox_inches='tight')
